...
module.exports = class extends Component {
    render() {
        ...
        return <Fragment>
            {(indexAt != undefined & indexAt == 0) ?
                <AdsenseX config={config} display={index_zero_position_ad}/> : null}
                ...
                <article class={`card-content article${'direction' in page ? ' ' + page.direction : ''}`}
                    ...
                    {page.layout !== 'page' ? <div class="article-meta size-small is-uppercase level is-mobile">
                        <div class="level-left">
                            ...
                            {/*</span> : null}*/}
                            {showComment ?
                                <a class="commentCountImg"
                                   href={`${url_for(page.link || page.path)}#comment-container`}><span
                                    class="display-none-class">{id}</span><i class="far fa-comment-dots"/>&nbsp;<span
                                    class="commentCount" id={md5Id}>99+</span>&nbsp;&nbsp;</a> : null}
                            {/* Read time */}
                            ...
                            {/*</span> : null}*/}
                            {article && article.readtime && article.readtime === true ? <span class="level-item">
                                <i class="far fa-clock">&nbsp;</i>{timeStr} &nbsp;<i
                                class="fas fa-pencil-alt">&nbsp;</i>{wordsCount}&nbsp;k
                                </span> : null}
                            {/* Visitor counter */}
                    ...
                    {index && indexShowTagsCat ? <div class="index-category-tag">
                        ...
                        {page.categories && page.categories.length ? <div class="level-item">
                            {(() => {
                                ...
                                page.categories.forEach((category, i) => {
                                    categories.push(<a class="article-more button is-small link-muted index-categories"
                            ...
                            <div class="level-item">
                                {(() => {
                                    ...
                                    page.tags.forEach((tag, i) => {
                                        tags.push(<a class="article-more button is-small link-muted index-tags"
                    ...
                    {!index && page.layout == 'post' && mysite !== undefined ?
                        <RecommendPosts.Cacheable config={config} curPost={page} helper={helper}
                                                  mysite={mysite}/> : null}
                    {/* Share button */}
                    {!index ? <Share config={config} page={page} helper={helper}/> : null}
                </article>
            </div>
            {index && index_ad_positions && (index_ad_positions.indexOf(indexAt) > -1) ?
                <AdsenseX config={config} display={index_ad_positions != undefined}/> : null}
            {/* Donate button */}
